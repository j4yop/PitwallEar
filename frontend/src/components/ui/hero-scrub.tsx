...
const PIN_VH_MULTIPLE = 0.4; // Tightened further — minimal scroll-through
    ...
    const ctx = gsap.context(() => {
      ...
      const master = gsap.timeline({
        scrollTrigger: {
          ...
          onUpdate: (self) => {
            // Map the whole progress curve across the (now small) section
            const p = self.progress;
            const mapped = gsap.utils.clamp(0, 1, p);
